...
function runPython(pythonBin: string, pdfPath: string): Promise<ParsedPdfResult> {
  return new Promise((resolve, reject) => {
    ...
    py.on("close", (code) => {
      ...
      try {
        resolve(JSON.parse(out) as ParsedPdfResult);
      } catch (e) {
        reject(e);
      }
    });
...
export async function runPdfParseWithFallback(pdfPath: string): Promise<ParsedPdfResult> {
  const candidates = [
